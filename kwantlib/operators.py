...
def cross_moving_average(        
    ...
    match type(signal):
        case pd.Series:
            return _cross_moving_average_ds(signal, smooth_params, lookback_params, is_ewm)
        case pd.DataFrame:
            return _cross_moving_average_df(signal, smooth_params, lookback_params, is_ewm, skipna, n_jobs)
        case _:
...
def _ranking_df(signal:pd.DataFrame, k:int, n_jobs:int) -> pd.DataFrame:
    ...
    with mp.Pool(n_jobs) as pool:
        results = pool.starmap(_ranking_row, tasks)
    return pd.concat([res for res in results], axis = 1)

def ranking(
        signal:pd.DataFrame, k_values:Iterable[int] = (3, 5, 7,), n_jobs:int = mp.cpu_count() - 2
    ) -> pd.DataFrame: 
    """
    ...
    """
    return pd.concat({
        f'{k}': _ranking_df(signal.ffill(), k, n_jobs)
        for k in k_values
...
def _markovitz(
        ...
        freq_retraining:int = 1, n_jobs:int = mp.cpu_count() - 2
    ) -> pd.DataFrame:
    """ 
    ...
    markovitz_func = {  
        'maxsharpe': _markovitz_maxsharpe,
        'minvol': _markovitz_minvol,
    }[method]
...
def markovitz(
    ...
    match level:
        case 'cross asset':
            return _markovitz(pnl, l2_reg, method, freq_retraining, n_jobs)
        case 'per asset':
            return pd.concat({
                col: _markovitz(
                    pnl.loc[:, col].dropna(), l2_reg, method, freq_retraining, n_jobs
                )
